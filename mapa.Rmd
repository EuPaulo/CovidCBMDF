...
## Mapa de casos no Distrito Federal
...
Mapa de casos confirmados no Distrito Federal, de acordo com os [boletins epidemiológicos](http://www.saude.df.gov.br/informativos-do-centro-de-operacoes-de-emergencia-coe/) publicado pela Secretaria de Estado de Saúde do DF. Atualizado em `r as.character(max(dadosMapa$Data))`.
...
leaflet() %>%
  ...
  addCircleMarkers(~longitude, ~latitude, radius=1,
    fillOpacity=0.4, color = ~situacao,  clusterOptions = markerClusterOptions(), data = dadosMapa[dadosMapa$Data == max(dadosMapa$Data),])
```